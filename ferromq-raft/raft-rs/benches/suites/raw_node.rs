...
use ferromq_raft_core::eraftpb::{ConfState, Entry, Message, Snapshot, SnapshotMetadata};
use ferromq_raft_core::{storage::MemStorage, Config, RawNode};
...
pub fn bench_raw_node_new(c: &mut Criterion) {
    let bench = |b: &mut Bencher| {
        let logger = ferromq_raft_core::default_logger();
