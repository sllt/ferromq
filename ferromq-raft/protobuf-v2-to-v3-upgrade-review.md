...
```
ferromq-raft (build.rs uses tonic-build 0.9)   ← 不受影响，使用独立的 prost/tonic 生态
  └─ tikv-raft (= ferromq-raft-core 0.8.0)
       └─ raft-proto (= ferromq-raft-proto 0.8.0)
            ├─ protobuf = "3.7.2"
            └─ protobuf-build (= ferromq-protobuf-build 0.16.0)
                 ├─ protobuf = "3.7.2"
