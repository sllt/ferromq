...
[![crates.io](https://img.shields.io/crates/v/ferromq-bridge-ingress-pulsar.svg)](https://crates.io/crates/ferromq-bridge-ingress-pulsar)
[![license](https://img.shields.io/crates/l/ferromq-bridge-ingress-pulsar.svg)](https://github.com/rmqtt/rmqtt/blob/master/LICENSE)

## Overview

`ferromq-bridge-ingress-pulsar` is an FerroMQ plugin that consumes messages from Apache Pulsar topics and publishes them to the local FerroMQ broker. It acts as an ingress bridge, bridging Pulsar messages into MQTT with comprehensive subscription and message transformation capabilities.
...
```toml
[dependencies]
ferromq-bridge-ingress-pulsar = { version = "*", features = ["plugin"] }
```

### Register Plugin

Add the plugin to your FerroMQ node configuration:

```toml
[[plugins]]
name = "ferromq-bridge-ingress-pulsar"
register = true
```
...
name = "ferromq-bridge-ingress-pulsar"
register = true
config_file = "/path/to/ferromq-bridge-ingress-pulsar.toml"
```
