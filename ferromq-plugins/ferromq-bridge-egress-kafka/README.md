...
[![crates.io](https://img.shields.io/crates/v/ferromq-bridge-egress-kafka.svg)](https://crates.io/crates/ferromq-bridge-egress-kafka)
[![license](https://img.shields.io/crates/l/ferromq-bridge-egress-kafka.svg)](https://github.com/rmqtt/rmqtt/blob/master/LICENSE)

## Overview

`ferromq-bridge-egress-kafka` is an FerroMQ plugin that forwards local MQTT publish messages to one or more Apache Kafka topics. It acts as an egress bridge, subscribing to local MQTT topics and producing the messages to Kafka with topic transformation support.
...
```toml
[dependencies]
ferromq-bridge-egress-kafka = { version = "*", features = ["plugin"] }
```
...
```toml
[[plugins]]
name = "ferromq-bridge-egress-kafka"
register = true
```
...
name = "ferromq-bridge-egress-kafka"
register = true
config_file = "/path/to/ferromq-bridge-egress-kafka.toml"
```
