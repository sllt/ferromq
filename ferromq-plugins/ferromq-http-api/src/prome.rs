...
impl MonitorData {
    fn new(typ: PrometheusDataType) -> Option<Self> {
        ...
        let nodes_gauge_vec =
            register_gauge_vec_with_registry!("ferromq_nodes", "nodes status", &["node", "item"], reg)
                .ok()?;

        let stats_gauge_vec =
            register_int_gauge_vec_with_registry!("ferromq_stats", "status data", &["node", "item"], reg)
                .ok()?;

        let metrics_gauge_vec =
            register_int_gauge_vec_with_registry!("ferromq_metrics", "metrics data", &["node", "item"], reg)
                .ok()?;
