...
[![crates.io](https://img.shields.io/crates/v/ferromq-bridge-ingress-kafka.svg)](https://crates.io/crates/ferromq-bridge-ingress-kafka)
[![license](https://img.shields.io/crates/l/ferromq-bridge-ingress-kafka.svg)](https://github.com/rmqtt/rmqtt/blob/master/LICENSE)

## Overview

`ferromq-bridge-ingress-kafka` is an FerroMQ plugin that consumes messages from one or more Apache Kafka topics and publishes them to the local FerroMQ broker. It acts as an ingress bridge, bridging Kafka messages into MQTT with flexible topic mapping and consumer group management.
...
```toml
[dependencies]
ferromq-bridge-ingress-kafka = { version = "*", features = ["plugin"] }
```
...
```toml
[[plugins]]
name = "ferromq-bridge-ingress-kafka"
register = true
```
...
name = "ferromq-bridge-ingress-kafka"
register = true
config_file = "/path/to/ferromq-bridge-ingress-kafka.toml"
```
