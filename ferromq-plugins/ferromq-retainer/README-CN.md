...
在 `ferromqd/Cargo.toml` 中添加依赖，或通过 `ferromq-plugins` 元 crate 启用：
...
配置文件：`ferromq-retainer.toml`（位于插件配置目录）。通过 `scx.plugins.load_config_default::<PluginConfig>("ferromq-retainer")` 加载。

| 选项 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `storage.type` | `string` | `"ram"` | 存储后端类型：`ram`、`sled`、`redis` |
| `storage.sled.path` | `string` | `"/var/log/ferromq/.cache/retain/{node}"` | Sled 数据库路径（支持 `{node}` 占位符替换为节点 ID） |
...
插件通过 `scx.plugins.load_config_default::<PluginConfig>("ferromq-retainer")` 加载配置，支持以下来源：

1. `{plugins.dir}/ferromq-retainer.toml`（文件，可选——文件缺失时使用默认值）
2. `ferromq_plugin_ferromq_retainer_*` 环境变量（将 TOML 键映射为带下划线前缀的环境变量）
...
`ferromq`（features: `plugin`、`retain`）、`ferromq-storage`、`serde`、`tokio`、`sled`（可选）、`redis`（可选）
