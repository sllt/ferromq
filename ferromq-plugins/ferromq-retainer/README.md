...
[![crates.io](https://img.shields.io/crates/v/ferromq-retainer.svg)](https://crates.io/crates/ferromq-retainer)
...
Add the dependency in `ferromqd/Cargo.toml` or enable via the `ferromq-plugins` meta-crate:

```toml
# Direct dependency
ferromq-retainer = { version = "0.22", features = ["ram"] }

# Or via meta-crate
ferromq-plugins = { version = "0.22", features = ["retainer-ram"] }
```
...
ferromq_retainer::register_named(&scx, "ferromq-retainer", true, false).await?;
```
...
File: `ferromq-retainer.toml` (in the plugin config directory). Loaded via `scx.plugins.load_config_default::<PluginConfig>("ferromq-retainer")`.

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `storage.type` | `string` | `"ram"` | Backend type: `ram`, `sled`, `redis` |
| `storage.sled.path` | `string` | `"/var/log/ferromq/.cache/retain/{node}"` | Sled database path (supports `{node}` placeholder for node ID) |
...
The plugin loads configuration via `scx.plugins.load_config_default::<PluginConfig>("ferromq-retainer")`, supporting:

1. `{plugins.dir}/ferromq-retainer.toml` (file, optional — falls back to defaults if missing)
2. `ferromq_plugin_ferromq_retainer_*` environment variables (maps TOML keys to env vars with underscore prefix)
...
storage.type = "sled"
storage.sled.path = "/var/log/ferromq/.cache/retain/{node}"
...
`ferromq` (features: `plugin`, `retain`), `ferromq-storage`, `serde`, `tokio`, `sled` (optional), `redis` (optional)
