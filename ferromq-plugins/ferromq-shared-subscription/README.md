...
# ferromq-shared-subscription

[![crates.io](https://img.shields.io/crates/v/ferromq-shared-subscription.svg)](https://crates.io/crates/ferromq-shared-subscription)

Shared subscription plugin for FerroMQ. Implements subscriber selection strategies for `$share/{group}/{topic}` subscriptions.
...
Add to the `plugins.default_startups` list in `ferromq.toml`:
...
```rust
ferromq_shared_subscription::register(register_fn);
```

## Configuration

File: `ferromq-shared-subscription.toml`
...
- `ferromq` (features `plugin`, `shared-subscription`)
