...
[![crates.io](https://img.shields.io/crates/v/ferromq-bridge-egress-nats.svg)](https://crates.io/crates/ferromq-bridge-egress-nats)
[![license](https://img.shields.io/crates/l/ferromq-bridge-egress-nats.svg)](https://github.com/rmqtt/rmqtt/blob/master/LICENSE)

## Overview

`ferromq-bridge-egress-nats` is an FerroMQ plugin that forwards local MQTT publish messages to one or more NATS subjects. It acts as an egress bridge, subscribing to local MQTT topics and publishing messages to NATS with topic transformation support.
...
```toml
[dependencies]
ferromq-bridge-egress-nats = { version = "*", features = ["plugin"] }
```
...
```toml
[[plugins]]
name = "ferromq-bridge-egress-nats"
register = true
```
...
name = "ferromq-bridge-egress-nats"
register = true
config_file = "/path/to/ferromq-bridge-egress-nats.toml"
```
