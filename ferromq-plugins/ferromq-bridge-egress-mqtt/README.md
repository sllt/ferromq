...
[![crates.io](https://img.shields.io/crates/v/ferromq-bridge-egress-mqtt.svg)](https://crates.io/crates/ferromq-bridge-egress-mqtt)
[![license](https://img.shields.io/crates/l/ferromq-bridge-egress-mqtt.svg)](https://github.com/rmqtt/rmqtt/blob/master/LICENSE)

## Overview

`ferromq-bridge-egress-mqtt` is an FerroMQ plugin that forwards local MQTT publish messages to one or more remote MQTT brokers. It acts as an egress bridge, subscribing to local topics and republishing matching messages to the configured remote brokers with topic transformation support.
...
```toml
[dependencies]
ferromq-bridge-egress-mqtt = { version = "*", features = ["plugin"] }
```
...
```toml
[[plugins]]
name = "ferromq-bridge-egress-mqtt"
register = true
```
...
name = "ferromq-bridge-egress-mqtt"
register = true
config_file = "/path/to/ferromq-bridge-egress-mqtt.toml"
```
