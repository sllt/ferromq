...
[![crates.io](https://img.shields.io/crates/v/ferromq-session-storage.svg)](https://crates.io/crates/ferromq-session-storage)
...
File: `ferromq-session-storage.toml`
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `storage.sled.path` | string | `"/var/log/ferromq/.cache/session/{node}"` | Sled database file path (`{node}` = node ID placeholder) |
...
`ferromq` (feature `plugin`), `sled`, `redis`, `tokio`
