...
use ferromq::codec::v5::{PublishProperties, UserProperties};
use ferromq::context::ServerContext;
use ferromq::types::{CodecPublish, Publish};
use ferromq::utils::timestamp_millis;
use ferromq::{
...
async fn send_publish(scx: ServerContext, cfg: Arc<Bridge>, entry_idx: usize, f: From, p: BridgePublish) {
    ...
    let msg = match p {
        BridgePublish::V3(p) => ferromq::codec::types::Publish {
            dup: false,
        ...
        BridgePublish::V5(p) => ferromq::codec::types::Publish {
            dup: false,
            retain: entry.local.make_retain(p.retain),
            qos: entry.local.make_qos(p.qos),
            topic: entry.local.make_topic(p.topic.as_str()),
