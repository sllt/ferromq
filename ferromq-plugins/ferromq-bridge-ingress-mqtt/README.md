...
[![crates.io](https://img.shields.io/crates/v/ferromq-bridge-ingress-mqtt.svg)](https://crates.io/crates/ferromq-bridge-ingress-mqtt)
[![license](https://img.shields.io/crates/l/ferromq-bridge-ingress-mqtt.svg)](https://github.com/rmqtt/rmqtt/blob/master/LICENSE)

## Overview

`ferromq-bridge-ingress-mqtt` is an FerroMQ plugin that connects to a remote MQTT broker and forwards messages to the local FerroMQ broker. It acts as an ingress bridge, subscribing to remote topics and republishing the received messages locally with topic transformation support.
...
```toml
[dependencies]
ferromq-bridge-ingress-mqtt = { version = "*", features = ["plugin"] }
```
...
```toml
[[plugins]]
name = "ferromq-bridge-ingress-mqtt"
register = true
```
...
name = "ferromq-bridge-ingress-mqtt"
register = true
config_file = "/path/to/ferromq-bridge-ingress-mqtt.toml"
```
