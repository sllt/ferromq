...
use ferromq::utils::{deserialize_duration, deserialize_duration_option, NodeAddr};
use ferromq::{
    args::CommandArgs,
    grpc::MessageType,
    types::{Addr, NodeId},
...
use ferromq_raft::ReadOnlyOption;
...
///
/// Mirrors `ferromq_raft::Config` fields, allowing customization of gRPC
...
impl RaftConfig {
    pub(crate) fn to_raft_config(&self) -> ferromq_raft::Config {
        let mut cfg = ferromq_raft::Config { ..Default::default() };
