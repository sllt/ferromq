...
[![crates.io](https://img.shields.io/crates/v/ferromq-cluster-raft.svg)](https://crates.io/crates/ferromq-cluster-raft)
...
```toml
[dependencies]
ferromq-cluster-raft = "0.21"
```

Requires `ferromq` features: `plugin`, `grpc`, `stats`, `msgstore`.

```rust
ferromq_cluster_raft::register(&scx, true, false).await?;
```

CLI example:

```bash
ferromqd --id 1 --plugins-default-startups "ferromq-cluster-raft" \
...
File: `ferromq-cluster-raft.toml`
