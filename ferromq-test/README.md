...
**ferromq-test** is the industrial-grade test harness, chaos engineering, and benchmarking engine for the [FerroMQ](https://github.com/rmqtt/rmqtt) MQTT broker.

The build artifact `ferromq_harness` is a standalone executable that provides functional testing, stress testing, chaos testing, and outputs structured test reports.

## ✨ Features

- **Custom MQTT Client** — Zero third-party MQTT dependency, complete MQTT 3.1 / 3.1.1 / 5.0 protocol stack
- **Broker Lifecycle Management** — Auto start/stop/restart `ferromqd` process with TCP health checks
...
Artifact located at `target/release/ferromq_harness` (`ferromq_harness.exe` on Windows).
...
The program will auto-locate `target/release/ferromqd` and start the broker.
By default it uses the **self-contained config**
`ferromq-test/configs/default/ferromq.toml` (independent from the repository-root
`ferromq.toml` / `ferromq-plugins/*.toml`; all TCP/TLS/WS/WSS/QUIC listeners are
...
```bash
# JSON report
./target/release/ferromq_harness --no-broker --json report.json

# HTML report
./target/release/ferromq_harness --no-broker --html report.html

# Both formats
./target/release/ferromq_harness --no-broker --json report.json --html report.html
```
...
./target/release/ferromq_harness --workspace . --suites stress

# Multiple suites
./target/release/ferromq_harness --workspace . --suites functional_v3 --suites functional_v311
```
...
All test broker configs live under `ferromq-test/configs/<name>/` and are
**self-contained** (main config + own `plugins/` sub-dir), independent from
the repository-root `ferromq.toml` / `ferromq-plugins/*.toml`:

```
configs/
  default/                  # default config (used when --config is omitted)
    ferromq.toml              #   based on the repo-root config, all listeners kept
...
./target/release/ferromqd -f ferromq-test/configs/pubrel-collision-cluster/node1/ferromq.toml
./target/release/ferromqd -f ferromq-test/configs/pubrel-collision-cluster/node2/ferromq.toml

# terminal 3: run the cluster reproduction suite
./target/release/ferromq_harness --no-broker --addr 127.0.0.1:1884 --suites functional_v5_cluster --workers 1
```
...
```bash
RUSTUP_TOOLCHAIN=1.97 cargo build -p ferromqd
RUSTUP_TOOLCHAIN=1.97 cargo build -p ferromq-test
...
rm -rf ferromq-test/configs/{session-sled,session-sled-stress,cluster-broadcast-sled,cluster-broadcast-sled-stress,cluster-raft-sled,cluster-raft-sled-stress}/.sled
...
ferromq-test/
  src/
    main.rs                      # ferromq_harness entry point, suite registration
  ...
  configs/                       # Test broker configs (all self-contained)
    default/                     #   default config: ferromq.toml + plugins/ (retainer/shared-subscription/http-api)
    retain-disabled/             #   retainer plugin NOT loaded (Retain Available = 0)
