...
FerroMQ 的工业级验证与压测核心引擎（Test Harness + Chaos + Benchmark）。编译产物 `ferromq_harness` 作为独立可执行程序，提供功能测试、压力测试、混沌测试，并输出结构化测试报告。

## ✨ 特性

- **自研 MQTT 客户端** — 零第三方 MQTT 依赖，完整实现 MQTT 3.1 / 3.1.1 / 5.0 协议栈
- **Broker 生命周期管理** — 自动启动/停止/重启 ferromqd 进程，TCP 健康检查
...
产物位于 `target/release/ferromq_harness`（Windows 下为 `ferromq_harness.exe`）。
...
程序会自动查找 `target/release/ferromqd` 并启动 Broker，**默认使用自包含配置
`ferromq-test/configs/default/ferromq.toml`**（不依赖仓库根的 `ferromq.toml` /
`ferromq-plugins/*.toml`；保留 TCP/TLS/WS/WSS/QUIC 全部监听，便于后续添加
TLS/WS/QUIC 专项测试）。
...
所有测试用 broker 配置均位于 `ferromq-test/configs/<name>/`，**自包含**（主配置 +
自身 `plugins/` 子目录），不依赖仓库根的 `ferromq.toml` / `ferromq-plugins/*.toml`：

```
configs/
  default/                  # 默认配置（未指定 --config 时使用）
    ferromq.toml              #   以仓库根 ferromq.toml 为蓝本，保留全部 listener
...
ferromq-test/
  src/
    main.rs                      # ferromq_harness 入口，套件注册
  ...
  configs/                       # 测试用 broker 配置（全部自包含）
    default/                     #   默认配置：ferromq.toml + plugins/（retainer/shared-subscription/http-api）
    retain-disabled/             #   不加载 retainer 插件（Retain Available = 0）
