...
//!
//! Current FerroMQ behaviour (after the MQTT-3.1.3-6 fix in `ferromq/src/v3.rs`):
//! the check exists at the codec layer — `ferromq-codec/src/v3/decode.rs`
//! rejects an empty ClientId with `DecodeError::InvalidClientId` when
//! CleanSession is 0. `ferromq/src/v3.rs` maps that decode error to a CONNACK
...
use ferromq_codec::types::{Protocol, MQTT_LEVEL_311};
use ferromq_codec::v3::{Connect, ConnectAckReason, Packet};
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result: Result<(), anyhow::Error> = rt.block_on(async {
            ...
            if p.reason != ConnectAckReason::IdentifierRejected {
                // Actual FerroMQ behaviour before the fix: rejected with 0x03.
