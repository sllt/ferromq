//! MQTT v5.0 Retained Message functional tests
//!
...
use ferromq_codec::v5::{LastWill, QoS};
...
pub struct RetainV5StoreAndDeliverTest;

impl TestCase for RetainV5StoreAndDeliverTest {
    fn name(&self) -> &str {
        "retain_v5_store_and_deliver"
    }
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result = rt.block_on(async {
            ...
            let topic = "test/v5/retain/store";
...
pub struct RetainV5EmptyDeleteTest;

impl TestCase for RetainV5EmptyDeleteTest {
    fn name(&self) -> &str {
        "retain_v5_empty_payload_deletes"
    }
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result = rt.block_on(async {
            ...
            let topic = "test/v5/retain/delete";
...
pub struct RetainV5OverwriteTest;

impl TestCase for RetainV5OverwriteTest {
    fn name(&self) -> &str {
        "retain_v5_overwrite"
    }
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result = rt.block_on(async {
            ...
            let topic = "test/v5/retain/overwrite";
...
/// subscriber with RETAIN=0. [MQTT-3.3.1-10]
pub struct RetainV5LiveNotRetainedTest;

impl TestCase for RetainV5LiveNotRetainedTest {
    fn name(&self) -> &str {
        "retain_v5_live_message_not_retained"
    }
...
pub struct RetainV5WillTest;

impl TestCase for RetainV5WillTest {
    fn name(&self) -> &str {
        "retain_v5_will"
    }
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let result = rt.block_on(async {
            let will_topic = "test/v5/retain/will";
            let will = LastWill {
                qos: QoS::AtLeastOnce,
                retain: true,
            ...
            if let Ok(c) = crate::mqtt::v5::MqttV5Client::connect(
                &ctx.config.broker_addr,
                "v5-retain-will-cleanup",
                ctx.config.connect_timeout,
            )
            .await
            {
