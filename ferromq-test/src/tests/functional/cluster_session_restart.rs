...
//! once with `ferromq-cluster-broadcast` (two nodes), once with
//! `ferromq-cluster-raft` (three nodes):
//!
...
//!   (`ferromq-cluster-broadcast/src/router.rs`), so the #475 defect (rebuild
...
//!   (`ferromq-cluster-raft/src/router.rs` `mailbox.send_proposal`), so
...
//! # Configs
//!
//! - broadcast: `ferromq-test/configs/cluster-broadcast-sled/node{1,2}/`
//!   (MQTT 1886/1887, gRPC 5366/5367)
//! - raft:      `ferromq-test/configs/cluster-raft-sled/node{1,2,3}/`
//!   (MQTT 1888/1889/1890, gRPC 5368/5369/5370, raft 6008/6009/6010;
//!   `leader_id = 1` so nodes 2/3 wait for node 1 to lead)
//!
//! # Run
//!
//! Prerequisites: `cargo build -p ferromqd && cargo build -p ferromq-test`
...
/// Locate the ferromqd binary for the self-managed cluster nodes.
///
/// Prefers the **debug** build (so a `cargo build -p ferromqd` right before the
...
pub(crate) fn ferromqd_binary() -> PathBuf {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|| PathBuf::from("."));
    for dir in ["target/debug", "target/release"] {
        for name in ["ferromqd.exe", "ferromqd", "ferromqd.exe", "ferromqd"] {
...
/// issue #475 cross-node reproduction with `ferromq-cluster-broadcast`.
...
/// issue #475 cross-node reproduction with `ferromq-cluster-raft`.
...
/// issue #475 whole-cluster restart reproduction with `ferromq-cluster-broadcast`:
...
/// issue #475 whole-cluster restart reproduction with `ferromq-cluster-raft`
