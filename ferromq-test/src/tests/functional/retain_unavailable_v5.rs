...
//! FerroMQ advertises `Retain Available = 0` in its CONNACK (retain feature
...
//! ferromq-retainer plugin, see `ferromq-test/configs/retain-disabled/ferromq.toml`):
//!
//! ```text
//! ferromq_harness \
//!   --binary target/release/ferromqd \
//!   --config ferromq-test/configs/retain-disabled/ferromq.toml \
//!   --workspace . \
...
use ferromq_codec::types::QoS;
use ferromq_codec::v5::{Connect, ConnectAckReason, LastWill, Packet};
use uuid::Uuid;
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result: Result<Outcome, anyhow::Error> = rt.block_on(async {
            ...
            if p.reason != ConnectAckReason::RetainNotSupported {
                // Actual FerroMQ behaviour (issue #457): accepted with 0x00.
