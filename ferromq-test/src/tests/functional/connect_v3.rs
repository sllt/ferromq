    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result = rt.block_on(async {
            ...
            assert_eq!(
                ...
                ferromq_codec::v3::ConnectAckReason::ConnectionAccepted,
...
impl TestCase for ConnectV3WithOptionsTest {
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result = rt.block_on(async {
            let will = ferromq_codec::v3::LastWill {
                qos: ferromq_codec::v3::QoS::AtLeastOnce,
                retain: false,
...
///
/// FerroMQ's default `max_clientid_len` is 65535 (v3.1.1 relaxed policy), so a
...
///
/// FerroMQ's default `max_clientid_len` is 65535, so a 65535-byte id sits
