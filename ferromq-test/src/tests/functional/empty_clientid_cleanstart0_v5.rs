...
//!
//! Current FerroMQ behaviour (defect under reproduction): the check itself
//! exists at the codec layer — `ferromq-codec/src/v5/packet/connect.rs` rejects
//! an empty ClientId with `DecodeError::InvalidClientId` when CleanStart is 0
//! (MQTT-3.1.3-8). However `ferromq/src/v5.rs` maps that decode error to a
...
use ferromq_codec::v5::{Connect, ConnectAckReason, Packet};
    ...
    fn execute(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result: Result<(), anyhow::Error> = rt.block_on(async {
            ...
            if p.reason != ConnectAckReason::ClientIdentifierNotValid {
                // Actual FerroMQ behaviour (the reproduced defect): the connection
