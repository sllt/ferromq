...
use ferromq_codec::v5::ConnectAckReason;
use ferromq_codec::v5::{
...
pub struct SubscribeAck {
    ...
    pub status: Vec<ferromq_codec::v5::SubscribeAckReason>,
}
...
impl MqttV5Client {
    ...
    pub async fn subscribe(&mut self, topic: &str, qos: QoSTest) -> Result<SubscribeAck> {
        self.subscribe_with_options(topic, qos, false, false, ferromq_codec::v5::RetainHandling::AtSubscribe)
            .await
    }
    ...
    pub async fn subscribe_with_options(
        ...
        retain_handling: ferromq_codec::v5::RetainHandling,
    ) -> Result<SubscribeAck> {
        let packet_id = NonZeroU16::new(u16::from(self.packet_id_counter.next()))
            .ok_or_else(|| anyhow!("packet id overflow"))?;

        let subscribe_pkt = PacketV5::Subscribe(ferromq_codec::v5::Subscribe {
            packet_id,
            id: None,
            user_properties: Vec::new(),
            topic_filters: vec![(
    ...
    pub async fn unsubscribe(&mut self, topic: &str) -> Result<()> {
        ...
        let unsub = PacketV5::Unsubscribe(ferromq_codec::v5::Unsubscribe {
            packet_id,
    ...
    pub async fn disconnect_with_reason(&self, reason_code: Option<u8>) -> Result<()> {
        ...
        let code = reason_code
            .and_then(|c| ferromq_codec::v5::DisconnectReasonCode::try_from(c).ok())
            .unwrap_or(ferromq_codec::v5::DisconnectReasonCode::NormalDisconnection);

        let disc = ferromq_codec::v5::Disconnect {
            reason_code: code,
    ...
    pub async fn disconnect_with_session_expiry(&self, expiry_secs: Option<u32>) -> Result<()> {
        ...
        let disc = ferromq_codec::v5::Disconnect {
            reason_code: ferromq_codec::v5::DisconnectReasonCode::NormalDisconnection,
