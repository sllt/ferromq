...
impl TestContext {
    ...
    /// i.e. whether retained messages are enabled (the `ferromq-retainer`
        ...
        rt.block_on(async {
            let (mut reader, mut writer) =
                crate::transport::tcp_v5::connect(&self.config.broker_addr, self.config.connect_timeout)
                    .await?;
            let connect = ferromq_codec::v5::Connect {
                clean_start: true,
                keep_alive: 60,
                client_id: ByteString::from(format!("retain-probe-{}", Uuid::new_v4().as_simple())),
                ..Default::default()
            };
            writer.send_packet(&ferromq_codec::v5::Packet::Connect(Box::new(connect))).await?;
            ...
            match pkt {
                ferromq_codec::v5::Packet::ConnectAck(ack) => Ok(ack.retain_available),
    ...
    ///
    /// If the broker does not support retained messages (the `ferromq-retainer`
    ...
    pub fn guard_retain_required(&self, name: &str, suite: &str, start: Instant) -> Option<TestResult> {
        match self.retain_available() {
            ...
            Ok(false) => Some(TestResult::passed_with_note(
                ...
                suite,
                start.elapsed(),
                "skipped: 'ferromq-retainer' plugin not enabled (Retain Available = 0)",
            )),
