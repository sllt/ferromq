...
//!
//! Starts ferromqd as a child process, manages its lifecycle,
...
const DEFAULT_CONFIG_REL: &str = "ferromq-test/configs/default/ferromq.toml";
...
impl BrokerProcess {
    ...
    /// and uses the self-contained `ferromq-test/configs/default/ferromq.toml`
    /// as the default config (never the repository-root ferromq.toml).
    ...
    pub fn find_binary(workspace_root: Option<&std::path::Path>) -> PathBuf {
        if let Some(root) = workspace_root {
            for dir in &["target/release", "target/debug"] {
                for name in ["ferromqd.exe", "ferromqd"] {
    ...
    pub fn start(&mut self) -> Result<(), anyhow::Error> {
        ...
        if let Some(ref config) = self.config_path {
            // ferromqd accepts `-f`/`--config` (config filename); `-c` is rejected.
