...
All notable changes to FerroMQ are documented in this file.

FerroMQ is based on [RMQTT](https://github.com/rmqtt/rmqtt) at upstream commit `e0e52d5982df5333ea5d5a908ea9b6061e27ed4d` (2026-09-02). Entries at 0.23.x and earlier describe that upstream history and were not rewritten.

## [0.24.0] - unreleased

### New Features

- **Dashboard session login (P3a)**: `ferromq-http-api` adds `POST /api/v1/auth/login|logout|init`, `GET /auth/me`, and `POST /auth/change-password`. Password hashes and API-key hashes use a durable per-node auth store; browser sessions remain process-local. The session cookie is HttpOnly / SameSite=Lax (optional Secure). `Authorization: Bearer <http_bearer_token>` remains an operator/admin credential. MQTT client auth plugins are unchanged.
...
- **Online configuration management (P4)**: `PUT /api/v1/plugins/{node}/{plugin}/config` writes plugin TOML atomically (JSON or TOML body), optional `?apply=reload`, and returns a diff plus `effective` (`hot` = already applied via plugin `load_config`, not a `ferromqd` restart; `reload` = file written, call `PUT .../config/reload`; `restart_required` = process restart needed). `POST .../config/validate` is a dry-run. Last-N backups: `GET .../config/versions`, `POST .../config/rollback/{version}`. GET config redacts `password`/`token`/`private_key`/`secret`/`jwt` keys unless `?reveal=1` and admin. Read-first `GET /api/v1/broker/config` (mqtt/listener/log); writable sections update `ferromq.toml` only and always report `restart_required`. Audit actions: `plugin_config_update`, `plugin_config_rollback`, `broker_config_update`, `broker_config_rollback`. Existing GET/reload/load/unload stay compatible.
- **Access control & integrations APIs (P5)**: structured REST on top of P4 plugin-config write for `ferromq-acl` rules (`/api/v1/acl/rules`), MQTT auth providers (`/api/v1/auth-providers/{http|jwt}` + `POST .../test` stubs), auto-subscription, topic-rewrite, webhooks (URL/rule CRUD, secret/userinfo redaction, TCP test ping with SSRF checks), and bridges (list/status from plugin `attrs()`, get/put config, load/unload). No dedicated blacklist plugin: `GET /api/v1/blacklist` reports the gap and points at ACL `connect` rules. Writes reuse `load_config` (`effective=hot` when possible). RBAC + audit (`acl_rule_*`, `webhook_*`, `bridge_*`, …).
- **Diagnostics & cluster ops (P6)**: `GET /api/v1/alarms` is a thin in-memory bus derived from health / feature inconsistency / unreachable peers (acknowledge via `POST /alarms/{id}/acknowledge`; not a native alarm plugin; lost on restart). `GET /logs`, `/trace`, `/slow-subs` return structured `available: false`. `GET /topic-metrics` is route-derived subscriber counts (no per-topic rates; `$SYS` listed only when `ferromq-sys-topic` is loaded). `GET /cluster` is read-only topology (`standalone` / `raft` / `broadcast`); `/brokers` and `/nodes` gain an additive `cluster` object. `POST /cluster/join` is always 501 (`Raft::join` is startup-only). `POST /cluster/leave` calls `ferromq-cluster-raft` `Plugin::send({op:leave})` → `Mailbox::leave` when that plugin is active, otherwise 501. Cluster writes always return per-node results. RBAC + audit (`alarm_acknowledge`, `cluster_join`, `cluster_leave`).

### Bug Fixes

- **Plugin config PUT preserves secrets**: generic plugin TOML writes deep-merge over the existing file (same idea as broker `merge_object`). Omitted or `***` secret keys (`hmac_secret`, `password`, tokens, …) keep the previous value; the literal `***` is never written. Structured ACL / auth-provider / bridge / webhook PUTs do the same for redacted fields (including ACL `who.password` and webhook URL userinfo).
- **`ferromq-http-api` config is admin-only**: operator+ can still write other plugins, but write / validate / rollback / reload of `ferromq-http-api` requires admin (it can set `http_bearer_token`, which authenticates as admin). Auth-related fields participate in `PluginConfig::changed` so `load_config` updates the live plugin configuration.
- **Session role is live**: after login, each request uses `DashboardUser.role` / `enabled`; a deleted user invalidates the session.
- **Change-password rotates sessions**: other sessions for that username are revoked; the caller receives a fresh cookie.
- **Viewer-first bootstrap** no longer consumes the empty-store bootstrap when admin credentials are configured, so a viewer login cannot lock out admin init.
- **TCP probe SSRF**: webhook / HTTP-auth tests resolve the hostname first and refuse blocked IPs (loopback / RFC1918 / metadata) unless `allow_private=1`. Still no HTTP GET.
- **Safe anonymous default**: when auth is unset, reads remain available as anonymous `viewer`; writes, secret reveal, user/key creation, and private-network probes are denied. Legacy anonymous admin requires `dashboard_allow_anonymous_admin = true`.
- **CSRF behind a reverse proxy**: cookie CSRF compares `Origin`/`Referer` to `Host`. Preserve the original public Host at the proxy; `X-Forwarded-Host` is not trusted. Non-browser clients can use Bearer / API key.
- **Config logging**: `PluginConfig` Debug / startup logs redact bearer and dashboard passwords.
- **Request-body logging**: when `http_request_log=true`, `/auth/*` bodies are omitted; other JSON/TOML bodies recursively redact password/token/secret/private_key/jwt/authorization/api_key/cookie fields and URL userinfo. Unparsed bodies log length only. `Authorization` / `Cookie` headers are never logged.

### Changed

- **React Dashboard embedded (P7)**: `ferromq-http-api` now rust-embeds the Vite production build of [`sllt/ferromq-dashboard`](https://github.com/sllt/ferromq-dashboard) from crate-local `dashboard-dist/` (source commit `92a5d01c7bfacb25e533f7e5b77e2aa3bbddb06b`, Hash Router + `base: './'`). The old Vue SPA under `dashboard/` is removed. `dashboard_static_dir` still overrides the embed when the directory exists. Dashboard responses add `Cache-Control` (no-cache for `index.html`, immutable for hashed `assets/*`), gzip when requested, and admin-SPA headers (`X-Content-Type-Options`, `X-Frame-Options`, pragmatic CSP). Rebuild the pinned commit with `./scripts/sync-dashboard-dist.sh`; pass `FERROMQ_DASHBOARD_REF=dev` only when intentionally refreshing.
...
- Default cluster cookie is `ferromqsecretcookie`. Mixed-version clusters with upstream RMQTT are not supported; stop the old cluster before starting FerroMQ.
- Default log/cache directory is `/var/log/ferromq`.
- Imported the exact dependency baselines `rmqtt-storage 0.11.1` and `rmqtt-raft 0.7.2` into the monorepo as `ferromq-storage` and `ferromq-raft`, including their internal Raft packages.
- Renamed storage-internal `__rmqtt*` keys to `__ferromq*`; existing persistent data requires an explicit migration and is not read transparently.
