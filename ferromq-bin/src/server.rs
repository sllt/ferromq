//! Binary entry point for the FerroMQ broker.
//!
...
use ferromq::args::CommandArgs;
use ferromq::context::{
    CircuitBreakerConfig, CountBasedWindowConfig, ServerContext, TimeBasedWindowConfig, WindowConfig,
};
use ferromq::net::{tls_provider, Builder};
use ferromq::node::Node;
use ferromq::server::MqttServer;
use ferromq::Result;
use ferromq_conf::{listener::Listener, Options, Settings};
...
async fn run() -> Result<()> {
    ...
    //print version
    log::info!("Starting FerroMQ (version={}, rustc={})", Node::version(), Node::rustc_version());
