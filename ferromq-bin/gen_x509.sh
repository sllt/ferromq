...
DNS="localhost ferromq.local"
# Trusted IP, none can be empty, separate multiples with English spaces.
IP="192.168.1.1 127.0.0.1"

# root ca
ROOT_KEY_FILE="ferromq-ca.key"
ROOT_CERT_FILE="ferromq-ca.pem"
ROOT_CERT_DAYS=3650
ROOT_RSA_KEY_BIT="rsa:4096"

# server ca
SERVER_KEY_FILE="ferromq.key"
SERVER_CERT_FILE="ferromq.pem"
SERVER_CERT_FULL_CHAIN_FILE="ferromq.fullchain.pem"
SERVER_CERT_DAYS=3650
SERVER_REQ_FILE="ferromq.req"
...
gen_root(){
    ...
    openssl req -nodes \
        ...
        -out ${ROOT_CERT_FILE} \
        -subj "/C=CN/O=FerroMQ/OU=FerroMQ/CN=FerroMQ Dev CA" \
        -sha256 \
        -batch \
...
gen_server(){
    ...
    openssl req -nodes \
        ...
        -out ${SERVER_REQ_FILE} \
        -subj "/C=CN/O=FerroMQ/OU=FerroMQ/CN=FerroMQ" \
        -sha256 \
        -batch \
        -config ${ROOT_CA_CONFIG_FILE}
...
gen_client(){
    ...
    openssl req -nodes \
        ...
        -out ${CLIENT_REQ_FILE} \
        -subj "/C=CN/O=FerroMQ/OU=FerroMQ/CN=FerroMQ Test Client" \
        -sha256 \
        -batch \
        -config ${ROOT_CA_CONFIG_FILE}
