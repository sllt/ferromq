...
use ferromq_codec::cert::CertInfo;
use ferromq_codec::error::{DecodeError, SendPacketError};
use ferromq_codec::v3::Codec as CodecV3;
use ferromq_codec::v5::Codec as CodecV5;
use ferromq_codec::version::{ProtocolVersion, VersionCodec};
use ferromq_codec::{MqttCodec, MqttPacket};
...
pub mod v3 {
    ...
    #[cfg(feature = "tls")]
    use ferromq_codec::cert::CertInfo;
    use ferromq_codec::error::DecodeError;
    use ferromq_codec::types::Publish;
    use ferromq_codec::v3::{Connect, ConnectAckReason, Packet as PacketV3, Packet};
    use ferromq_codec::{MqttCodec, MqttPacket};
    ...
    /// use tokio_util::codec::Framed;
    /// use ferromq_codec::{MqttCodec, types::Publish};
    /// use ferromq_net::{Builder,v3};
    ///
        ...
        Io: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        pub async fn send_subscribe_ack(
            ...
            status: Vec<ferromq_codec::v3::SubscribeReturnCode>,
        ) -> Result<()> {
        ...
        #[inline]
        pub async fn send_connect(&mut self, connect: ferromq_codec::v3::Connect) -> Result<()> {
        ...
        pub async fn send_connect_ack(
            ...
            self.send(PacketV3::ConnectAck(ferromq_codec::v3::ConnectAck { session_present, return_code }))
                .await
        }
        ...
        #[inline]
        pub async fn send(&mut self, packet: ferromq_codec::v3::Packet) -> Result<()> {
        ...
        #[inline]
        pub async fn recv(&mut self, tm: Duration) -> Result<Option<ferromq_codec::v3::Packet>> {
    ...
    where
        Io: AsyncRead + Unpin,
    {
        type Item = Result<ferromq_codec::v3::Packet>;
...
pub mod v5 {
    ...
    use ferromq_codec::error::DecodeError;
    use ferromq_codec::types::Publish;
    use ferromq_codec::v5::{Auth, Connect, Disconnect, Packet as PacketV5, Packet};
    use ferromq_codec::{MqttCodec, MqttPacket};
    ...
    /// use tokio_util::codec::Framed;
    /// use ferromq_codec::{MqttCodec, types::Publish};
    /// use ferromq_net::{Builder,v5};
    /// use ferromq_codec::v5::Connect;
    ///
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// let addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 1883);
    /// let stream = TcpStream::connect(addr).await?;
    ...
    /// mqtt_stream.send_auth(ferromq_codec::v5::Auth::default()).await?;
    /// # Ok(())
    /// # }
    /// ```
    impl<Io> MqttStream<Io>
        ...
        Io: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        /// Acknowledges a received publish (QoS 1)
        #[inline]
        pub async fn send_publish_ack(&mut self, ack: ferromq_codec::v5::PublishAck) -> Result<()> {
            self.send(PacketV5::PublishAck(ack)).await
        }

        /// Confirms receipt of a publish (QoS 2 step 1)
        #[inline]
        pub async fn send_publish_received(&mut self, ack: ferromq_codec::v5::PublishAck) -> Result<()> {
            self.send(PacketV5::PublishReceived(ack)).await
        }

        /// Releases a stored publish (QoS 2 step 2)
        #[inline]
        pub async fn send_publish_release(&mut self, ack2: ferromq_codec::v5::PublishAck2) -> Result<()> {
            self.send(PacketV5::PublishRelease(ack2)).await
        }

        /// Confirms publish completion (QoS 2 step 3)
        #[inline]
        pub async fn send_publish_complete(&mut self, ack2: ferromq_codec::v5::PublishAck2) -> Result<()> {
            self.send(PacketV5::PublishComplete(ack2)).await
        }

        /// Acknowledges a subscription request
        #[inline]
        pub async fn send_subscribe_ack(&mut self, ack: ferromq_codec::v5::SubscribeAck) -> Result<()> {
            self.send(PacketV5::SubscribeAck(ack)).await
        }

        /// Acknowledges an unsubscribe request
        #[inline]
        pub async fn send_unsubscribe_ack(&mut self, unack: ferromq_codec::v5::UnsubscribeAck) -> Result<()> {
            self.send(PacketV5::UnsubscribeAck(unack)).await
        }

        /// Initiates connection to the broker
        #[inline]
        pub async fn send_connect(&mut self, connect: ferromq_codec::v5::Connect) -> Result<()> {
            self.send(PacketV5::Connect(Box::new(connect))).await
        }

        /// Responds to connection request
        #[inline]
        pub async fn send_connect_ack(&mut self, ack: ferromq_codec::v5::ConnectAck) -> Result<()> {
        ...
        /// Generic packet sending method
        #[inline]
        pub async fn send(&mut self, packet: ferromq_codec::v5::Packet) -> Result<()> {
        ...
        /// Receives next packet with timeout
        #[inline]
        pub async fn recv(&mut self, tm: Duration) -> Result<Option<ferromq_codec::v5::Packet>> {
            match tokio::time::timeout(tm, self.next()).await {
                Ok(Some(Ok(msg))) => Ok(Some(msg)),
                Ok(Some(Err(e))) => Err(e),
                Ok(None) => Ok(None),
    ...
    where
        Io: AsyncRead + Unpin,
    {
        type Item = Result<ferromq_codec::v5::Packet>;
