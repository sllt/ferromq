//! Command-line argument parsing for the FerroMQ broker.
//!
...
use ferromq_utils::{NodeAddr, NodeId};

/// Command-line options for the FerroMQ broker.
///
/// Parsed from CLI arguments using `clap`. These values override settings
/// from the configuration file where applicable.
#[derive(Parser, Debug, Clone, Default)]
#[command(about = "FerroMQ MQTT Broker", disable_version_flag = true)]
pub struct Options {
...
mod tests {
    ...
    fn test_config_short() {
        let opts = Options::parse_from(["test", "-f", "ferromq.toml"]);
        assert_eq!(opts.cfg_name.as_deref(), Some("ferromq.toml"));
    }

    #[test]
    fn test_config_long() {
        let opts = Options::parse_from(["test", "--config", "/etc/ferromq/ferromq.toml"]);
        assert_eq!(opts.cfg_name.as_deref(), Some("/etc/ferromq/ferromq.toml"));
    }
    ...
    fn test_plugins_default_startups_single() {
        let opts = Options::parse_from(["test", "--plugins-default-startups", "ferromq-acl"]);
        assert_eq!(opts.plugins_default_startups, Some(vec!["ferromq-acl".to_string()]));
    }
    ...
    fn test_plugins_default_startups_multiple() {
        ...
        assert_eq!(
            ...
            Some(vec!["ferromq-acl".to_string(), "ferromq-retainer".to_string()])
        );
    }
    ...
    fn test_all_options() {
        ...
        assert_eq!(
            ...
            Some(&["ferromq-acl".to_string(), "ferromq-retainer".to_string()][..])
        );
