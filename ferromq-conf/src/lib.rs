...
//! Configuration management for the FerroMQ broker.
//!
...
impl Settings {
    fn new(opts: Options) -> Result<Self> {
        // Later sources win: default files < FERROMQ_* env < explicit -f/--config < CLI
        let mut builder = Config::builder()
            .add_source(File::with_name("/etc/ferromq/ferromq").required(false))
            .add_source(File::with_name("/etc/ferromq").required(false))
            .add_source(File::with_name("ferromq").required(false))
            .add_source(
                config::Environment::with_prefix("ferromq")
                    .try_parsing(true)
                    .list_separator(" ")
...
impl Node {
    fn cookie_default() -> String {
        "ferromqsecretcookie".into()
    }
    // fn crash_dump_default() -> String {
    //     "/var/log/ferromq/crash.dump".into()
    // }
}
...
pub struct Plugins {
    ...
    /// in `ferromq-bin/Cargo.toml` metadata.
    #[serde(default)]
...
impl Plugins {
    ...
    /// Environment variables with prefix `ferromq_plugin_{name}` are also sourced.
    ...
    fn load_config_with_required<'de, T: serde::Deserialize<'de>>(
        ...
        let mut env = config::Environment::with_prefix(&format!("ferromq_plugin_{}", name.replace('-', "_")));
...
///
/// Parsed from the `circuit_breaker.*` keys of `ferromq.toml`.
...
mod tests {
    ...
    fn explicit_config_is_loaded() {
        let dir = std::env::temp_dir().join(format!("ferromq-conf-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let cfg = dir.join("override.toml");
        std::fs::write(&cfg, "node.id = 7\n").unwrap();

        let opts = Options { cfg_name: Some(cfg.to_string_lossy().into_owned()), ..Options::default() };
        let settings = Settings::new(opts).expect("settings");
        assert_eq!(settings.node.id, 7);
