...
FerroMQ MQTT Broker 的集中式配置管理系统。
...
| `cookie` | `String` | `"ferromqsecretcookie"` | 集群认证 cookie |
...
| `dir` | `String` | `"/var/log/ferromq"` | 日志目录 |
| `file` | `String` | `"ferromq.log"` | 日志文件名 |
...
配置文件路径：`{plugins.dir}/{name}.toml`，环境变量前缀：`ferromq_plugin_{name}`。
...
3. `FERROMQ_*` 环境变量（如 `FERROMQ_NODE_ID=1`, `FERROMQ_RPC__SERVER_ADDR=0.0.0.0:5363`）
4. `-f` / `--config` 指定路径
5. CLI 参数
