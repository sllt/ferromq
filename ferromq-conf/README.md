...
[![crates.io page](https://img.shields.io/crates/v/ferromq-conf.svg)](https://crates.io/crates/ferromq-conf)
[![docs.rs page](https://docs.rs/ferromq-conf/badge.svg)](https://docs.rs/ferromq-conf/latest/ferromq_conf)

Centralized configuration management for the FerroMQ MQTT broker.
...
| `cookie` | `String` | `"ferromqsecretcookie"` | Cluster auth cookie |
...
| `dir` | `String` | `"/var/log/ferromq"` | Log directory |
| `file` | `String` | `"ferromq.log"` | Log filename |
...
Config files are loaded from `{plugins.dir}/{name}.toml`. Env prefix: `ferromq_plugin_{name}`.
...
3. `FERROMQ_*` env vars (e.g. `FERROMQ_NODE_ID=1`, `FERROMQ_RPC__SERVER_ADDR=0.0.0.0:5363`)
4. `-f` / `--config` specified path
5. CLI arguments
