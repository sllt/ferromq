//! # ferromq-storage example: MQTT Session Storage Simulation
//!
//! This example demonstrates the usage of all three core data structures in
//! `ferromq-storage`, using a simplified MQTT Broker session management scenario:
//!
...
#[cfg(feature = "redb")]
use ferromq_storage::RedbConfig;
#[cfg(feature = "sled")]
use ferromq_storage::SledConfig;
use ferromq_storage::{Config, DefaultStorageDB, List, Map, StorageList, StorageMap, StorageType};
...
async fn init_database() -> DefaultStorageDB {
    ...
    let temp_dir =
        std::env::temp_dir().join(format!("ferromq_storage_demo_{}", std::process::id()));
