...
WebHook is a feature provided by the [ferromq-web-hook](https://github.com/rmqtt/rmqtt/tree/master/ferromq-plugins/ferromq-web-hook) plugin, which allows for the notification of hook events in FerroMQ to a web service.

WebHook is internally implemented based on [hooks], but it is closer to the top layer. It captures various events in FerroMQ by attaching callback functions to hooks and forwards them to the web server configured in ferromq-web-hook.

Taking the example of the "client.connected" event, the event propagation process is as follows:

```bash
    Client      |    FerroMQ     |  ferromq-web-hook |   HTTP       +------------+
  =============>| - - - - - - -> - - - - - - - ->===========>  | Web Server |
                |    Broker    |                |  Request     +------------+
```
...
WebHook's event handling is unidirectional, as it only supports pushing events from FerroMQ to a web service and does not concern itself with the response from the web service.
...
```bash
plugins/ferromq-web-hook.toml
```

#### Plugin configuration options:

The configuration file for WebHook is located at [etc/plugins/ferromq-web-hook.toml](https://github.com/rmqtt/rmqtt/blob/master/ferromq-plugins/ferromq-web-hook.toml).
...
#urls = ["file:///var/log/ferromq/hook.log", "http://127.0.0.1:5656/mqtt/webhook"]
urls = ["file:///var/log/ferromq/hook.log"]
...
In `etc/plugins/ferromq-web-hook.toml`, trigger rules can be configured with the following format:
