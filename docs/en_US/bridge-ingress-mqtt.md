...
MQTT data bridging is a way to connect multiple FerroMQ clusters or other MQTT services. In ingress mode,
the local FerroMQ subscribes to topics from the bridged remote MQTT server and distributes the received
messages within the current cluster.


### Concurrent Connections:

FerroMQ allows multiple clients to simultaneously connect to the bridged MQTT server. When creating a bridge,
...
In ingress mode, if you have an FerroMQ cluster with multiple nodes and configure an ingress MQTT bridge to
...
each client in concurrent connections is assigned a unique client ID. FerroMQ automatically generates client
...
```bash
plugins/ferromq-bridge-ingress-mqtt.toml
```
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-ingress-mqtt` entry to the
