...
# FerroMQ Architecture Overview

This document describes the internal architecture of the FerroMQ MQTT broker, its components, module organization, and key design decisions.
...
graph TB
    ...
    subgraph "Network Layer (ferromq-net)"
    ...
    subgraph "Protocol Layer (ferromq-codec)"
    ...
    subgraph "Configuration (ferromq-conf)"
...
|-------|------|----------------|
| `ferromq-utils` | `ferromq-utils/` | Shared types (`Bytesize`, `NodeAddr`, `Counter`), serde helpers, timestamp/duration parsing |
| `ferromq-macros` | `ferromq-macros/` | Procedural macros: `#[derive(Metrics)]` for atomic counters, `#[derive(Plugin)]` for `PackageInfo` trait |
| `ferromq-codec` | `ferromq-codec/` | MQTT protocol encoder/decoder — v3.1, v3.1.1, v5.0 with version negotiation |
| `ferromq-storage` | `ferromq-storage/` | Unified asynchronous key-value storage abstraction for Sled, Redb, and Redis |
...
|-------|------|--------------|----------------|
| `ferromq-net` | `ferromq-net/` | `ferromq-codec`, `ferromq-utils` | Network layer: TCP/TLS/WS/QUIC listeners, connection accept, protocol dispatch |
| `ferromq-conf` | `ferromq-conf/` | `ferromq-codec`, `ferromq-net`, `ferromq-utils`, `config` crate | Configuration management: TOML parsing, CLI args, listener config |
| `ferromq-raft` | `ferromq-raft/` | `ferromq-raft-core`, Tonic, Postcard | Raft wrapper, node service, and cluster mailbox |

### Layer 3: Core Broker

| Crate | Path | Dependencies | Responsibility |
|-------|------|--------------|----------------|
| `ferromq` | `ferromq/` | All above + `ferromq-net`, `ferromq-codec`, `ferromq-utils`, `ferromq-macros` (optional), `rust-box`, `dashmap`, `tokio` | Core MQTT broker: session management, routing, hooks, plugins, clustering |

### Layer 4: Binaries

| Crate | Path | Responsibility |
|-------|------|----------------|
| `ferromqd` | `ferromq-bin/` | Production binary: CLI parsing → config → plugin registration → server start |
| `ferromq_harness` | `ferromq-test/` | Test harness: functional, stress, and chaos testing |

### Layer 5: Plugins

| Crate | Path | Responsibility |
|-------|------|----------------|
| `ferromq-plugins` | `ferromq-plugins/` | Meta-crate re-exporting all plugins behind feature flags |
| `ferromq-*` | `ferromq-plugins/ferromq-*/` | 26 individual plugin crates |

---

## Core Module Architecture (ferromq/src/)
...
```mermaid
flowchart LR
    A[" /etc/ferromq/ferromq.{toml,json}"] --> C[Config Builder]
    B["/etc/ferromq.{toml,json}"] --> C
    D["./ferromq.{toml,json}"] --> C
    E["-f / --config path"] --> C
    F["FERROMQ_* Env Vars"] --> C
    C --> G[Settings Singleton]
```

Plugin configs are loaded separately from `{plugins.dir}/{name}.toml` with `ferromq_plugin_{name}_*` environment variables.

---

## Feature Flags

The core broker (`ferromq`) uses Cargo feature flags to conditionally compile optional functionality:

| Feature | What it enables | Key Dependencies |
|---------|----------------|------------------|
| `default` | Minimal build (no extras) | — |
| `metrics` | Metrics collection | `ferromq-macros/metrics` |
| `stats` | Runtime statistics | — |
| `plugin` | Plugin system | `ferromq-macros/plugin` |
| `grpc` | Inter-node gRPC | `rust-box/handy-grpc`, `msgstore` |
| `tls` | TLS transport | `ferromq-net/tls` |
| `ws` | WebSocket transport | `ferromq-net/ws` |
| `quic` | QUIC transport | `ferromq-net/quic` |
...
Each plugin is a separate crate with optional compilation. The meta-crate (`ferromq-plugins`) re-exports all plugins behind feature flags, ensuring zero overhead for unused functionality.
