...
MQTT data bridging is a way to connect multiple FerroMQ clusters or other MQTT services. In egress mode, the local
FerroMQ forwards messages from the current cluster to the bridged remote MQTT server.

### Concurrent Connections:

FerroMQ allows multiple clients to connect simultaneously to the bridged MQTT server. When creating a bridge, you can
...
in the concurrent connections is assigned a unique client ID. To make client IDs predictable, FerroMQ automatically
...
```bash
plugins/ferromq-bridge-egress-mqtt.toml
```
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-egress-mqtt` entry to the
