...
Even if the **`store-session` plugin is not enabled**, FerroMQ still supports offline messages.
...
#### 2. Persistent storage (via `ferromq-session-storage` plugin)

When the **`ferromq-session-storage` plugin** is enabled, offline messages are persisted together with the session.
...
### Related configuration (in `ferromq.toml`)

Control offline-message related behavior in the main configuration file **`ferromq.toml`**:
...
* **Ordering & duplicates**: MQTT allows possible duplicate deliveries (especially with QoS ≥ 1 and reconnects). FerroMQ attempts to preserve order, but clients should handle idempotency.
