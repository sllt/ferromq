...
Upon restart of the FerroMQ service node, non-expired session basic information and subscription relationships will be
...
```bash
plugins/ferromq-session-storage.toml
```
...
##sled
storage.sled.path = "/var/log/ferromq/.cache/session/{node}"
...
Prefix configuration enables different FerroMQ nodes to use the same Redis storage service. `{node}` will be replaced with
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-session-storage` entry to the
