...
can comment on all the rules. After the rules file is changed, FerroMQ Broker needs to be restarted to make them take
effect.
</font>
</div>
...
## ferromq-acl.toml Writing rules

The rules in the `ferromq-acl.toml` file are matched from top to bottom in writing order.
...
After the `ferromq-acl.toml` modification is completed, it will not be automatically loaded into the FerroMQ Broker system,
but needs to be performed manually:

```bash
curl -X PUT "http://127.0.0.1:6060/api/v1/plugins/1/ferromq-acl/config/reload"
```

## Placeholders

The built-in `ferromq-acl.toml` supports only the following placeholders in the subject's field (the 4th position of the
tuple).
...
::: tip Only a few simple and general rules are contained in `ferromq-acl.toml` that make it a system-based ACL principle.
...
<div style="font-size:1.3em;">TIP<br></div>
<font style="color:#435364;font-size:1.1em;">
Only a few simple and general rules are contained in `ferromq-acl.toml` that make it a system-based ACL principle.
If you need to support complex, large amounts of ACL content, you should implement it in an authentication plugin.
</font>
</div>
