...
In ingress mode, the local FerroMQ consumes topics from the bridged remote *Apache-Pulsar* server and distributes the received
...
```bash
plugins/ferromq-bridge-ingress-pulsar.toml
```
...
##
## Value: String
#cert_chain_file = "./ferromq-bin/ferromq.fullchain.pem"
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-ingress-pulsar` entry to the
