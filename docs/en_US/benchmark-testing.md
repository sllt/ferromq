...
Broker Main Configuration (ferromq.toml):
```
##--------------------------------------------------------------------
## Plugins
##--------------------------------------------------------------------
...
```
podman network create compose_ferromq_bridge1
```
...
do
  ...
  mqtt-bench-a$i:
    ...
    command: v3 -C -c 25000 -E \"sub-a$i-{no}\" -S -t iot/a$i-{no} --addrs \"node1.ferromq.local:1883\" --ifaddrs \"a$i.ferromq-bench.local\"
    networks:
      ferromq_bridge1:
        aliases:
        - a$i.ferromq-bench.local"
...
do
  ...
  mqtt-bench-pub-a$i:
    image: rmqtt/rmqtt-bench:latest
    restart: on-failure:3
    command: v3 -C -c 1 -E \"pub-a$i-{no}\" -P -t iot/a$i-{no} -R 0 24999  -I 0 --addrs \"node1.ferromq.local:1883\" --ifaddrs \"a$i.ferromq-bench-pub.local\"
    networks:
      ferromq_bridge1:
        aliases:
        - a$i.ferromq-bench-pub.local"
...
```
The main difference from the single-node mode is the addition of the 'ferromq-cluster-raft' plugin. Modify the 'workers' and 'max_handshaking_limit' configuration options.

Raft Cluster Plugin Configuration(ferromq-cluster-raft.toml):
```
#Node GRPC service address list
node_grpc_addrs = ["1@node14.ferromq.local:5363","2@node15.ferromq.local:5363","3@node16.ferromq.local:5363"]
#Raft peer address list
raft_peer_addrs = ["1@node14.ferromq.local:16003","2@node15.ferromq.local:16003","3@node16.ferromq.local:16003"]
```
The address needs to match the address used in the Podman command.

Create a bridge network:
```
podman network create compose_ferromq_bridge11
```
...
do
  ...
  mqtt-bench-a$i:
    image: rmqtt/rmqtt-bench:latest
    restart: on-failure:3
    command: v3 -C -c 25000 -E \"sub-a$i-{no}\" -S -t iot/a$i-{no} --addrs \"node14.ferromq.local:1883\" \"node15.ferromq.local:1883\" \"node16.ferromq.local:1883\" --ifaddrs \"a$i.ferromq-bench.local\"
    networks:
      ferromq_bridge11:
        aliases:
        - a$i.ferromq-bench.local"
...
do
  ...
  mqtt-bench-pub-a$i:
    image: rmqtt/rmqtt-bench:latest
    restart: on-failure:3
    command: v3 -C -c 1 -E \"pub-a$i-{no}\" -P -t iot/a$i-{no} -R 0 24999  -I 0 --addrs \"node14.ferromq.local:1883\" \"node15.ferromq.local:1883\" \"node16.ferromq.local:1883\" --ifaddrs \"a$i.ferromq-bench-pub.local\"
    networks:
      ferromq_bridge11:
        aliases:
        - a$i.ferromq-bench-pub.local"
...
curl "http://127.0.0.1:7064/api/v1/stats/sum" | jq
{
  "nodes": {
    "1": {
      "name": "1@node14.ferromq.local:5363",
      "status": "Running"
    },
    "2": {
      "name": "2@node15.ferromq.local:5363",
      "status": "Running"
    },
    "3": {
      "name": "3@node16.ferromq.local:5363",
      "status": "Running"
    }
  },
  "stats": {
