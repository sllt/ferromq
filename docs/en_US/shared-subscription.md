...
This feature is provided by the [ferromq-shared-subscription](https://github.com/rmqtt/rmqtt/tree/master/ferromq-plugins/ferromq-shared-subscription) plugin.
...
```bash
plugins/ferromq-shared-subscription.toml
```
...
Shared subscriptions are automatically available on all listeners once the plugin is loaded. Add `ferromq-shared-subscription` to the `plugins.default_startups` list in `ferromq.toml`:
