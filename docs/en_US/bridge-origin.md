...
The `ferromq-bridge-origin` plugin is responsible for identifying whether an MQTT client originates from a bridge-ingress-mqtt or bridge-egress-mqtt connection.

When bridge plugins (such as `ferromq-bridge-ingress-mqtt` or `ferromq-bridge-egress-mqtt`) connect to remote MQTT brokers, they generate client IDs with identifiable markers. For example:
...
The `ferromq-bridge-origin` plugin listens for the `ClientConnected` hook event after a client successfully connects, checks the `client_id` against configurable markers, and writes the bridge origin information into `session.extra_attrs`. This metadata can then be consumed by other plugins for purposes such as anti-loop or routing decisions.
...
```bash
plugins/ferromq-bridge-origin.toml
```
...
The attribute key used to read the data must match the `attr_key` configured in `ferromq-bridge-origin` (defaults to `"bridge_origin"`):
...
These markers align with the auto-generated client ID patterns used by `ferromq-bridge-ingress-mqtt` and `ferromq-bridge-egress-mqtt` plugins.
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-origin` entry to the
