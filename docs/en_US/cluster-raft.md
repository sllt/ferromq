...
The *FerroMQ* cluster is implemented via the *ferromq-cluster-raft* plugin. Each node in the cluster runs an instance of
*FerroMQ* and communicates with other nodes in the cluster to share information such as client connection states and
...
The *FerroMQ* cluster maintains multiple replicas of data across different nodes to provide redundancy. Even if some
...
### *FerroMQ* Distributed Cluster Architecture

The core functionality of the *FerroMQ* distributed cluster is to forward and publish messages to subscribers, as
illustrated in the diagram below.

![Sample Diagram](../imgs/cluster-ps.png)

The *FerroMQ* cluster primarily maintains the following data structures:
...
*FerroMQ* maintains a subscription relationship table to store the mappings between topics and subscribers, ensuring that
...
```bash
plugins/ferromq-cluster-raft.toml
```
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-cluster-raft` entry to the
