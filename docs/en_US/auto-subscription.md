...
Auto Subscription allows *FerroMQ* to set multiple rules, subscribing the device to specified topics according to the
...
The Auto Subscription rules in *FerroMQ* need to be configured by the user, who can add multiple Auto Subscription rules as needed.
...
```bash
plugins/ferromq-auto-subscription.toml
```
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-auto-subscription` entry to the
