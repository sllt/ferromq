...
server to store client authentication credentials or session information. *FerroMQ* supports user authentication based on JWT.
...
Since the *FerroMQ* JWT authenticator only verifies the JWT signature and cannot guarantee the legitimacy of the client's
...
and issue a JWT for legitimate clients. The client will then use the issued JWT to connect to *FerroMQ*.

#### Access Control List (Optional)

If the JWT contains an `acl` field, *FerroMQ* will enforce access control for the client based on the permissions specified
...
##
## Value: File
public_key = "./ferromq-bin/jwt_public_key_rsa.pem"
...
#validate_claims.sub = "user@ferromq.local"
## Validate the token's issuer by comparing the iss claim to the known issuer.
#validate_claims.iss = ["https://example.com", "https://ferromq.local"]
 ...
 * Algorithm(encrypt): Specifies the encryption method for the JWT. Optional values: `hmac-based` or `public-key`:
   * If `hmac-based` is selected, the JWT will use a symmetric key to generate and verify the signature (supporting
     ...
     * Secret Base64 Encode(hmac_base64): Configure whether *FerroMQ* needs to perform Base64 decoding on the Secret before
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-auth-jwt` entry to the
