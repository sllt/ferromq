...
local *FerroMQ* forwards messages from the current cluster to the bridged remote *Reductstore* server.
...
```bash
plugins/ferromq-bridge-egress-reductstore.toml
```
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-egress-reductstore` entry to the
