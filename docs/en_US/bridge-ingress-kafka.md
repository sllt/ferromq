...
In ingress mode, the local FerroMQ consumes topics from the bridged remote *Apache-Kafka* server and distributes the received
messages within the current cluster.

If multiple nodes in an ingress FerroMQ cluster consume the same *Apache-Kafka* topic with different remote.group_id values,
...
```bash
plugins/ferromq-bridge-ingress-kafka.toml
```
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-ingress-kafka` entry to the
