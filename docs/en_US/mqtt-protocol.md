...
FerroMQ supports the following MQTT protocol versions and strictly follows the official specifications to ensure standardized message exchange between clients and the server while maintaining cross-version compatibility:
...
* FerroMQ supports **offline message queues**: QoS 1/2 messages are stored when clients are offline.
...
* FerroMQ provides detailed **error codes** for connection exceptions.
