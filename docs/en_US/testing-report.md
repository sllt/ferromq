...
This document provides the detailed test results for the FerroMQ MQTT broker, including interoperability testing against the paho.mqtt.testing suite, and performance benchmark data.

---

## Interoperability Testing

FerroMQ is tested against the official [paho.mqtt.testing](https://github.com/eclipse/paho.mqtt.testing) interoperability test suite.
...
# Start FerroMQ broker (separate terminal)
...
| `test_unsubscribe` | ✅ OK | — |
| `test_subscribe_failure` | ✅ OK | Requires ACL config: add `["deny", "all", "subscribe", ["test/nosubscribe"]]` at first line of `ferromq-acl.toml` |
| `test_zero_length_clientid` | ✅ OK | — |
...
| `test_will_delay` | ✅ OK | — |
| `test_server_keep_alive` | ✅ OK | Requires config: set `max_keepalive` to 60 in `ferromq.toml` |
...
The `ferromq-test` crate provides a custom test harness with additional test suites beyond paho:
...
> self-contained `ferromq-test/configs/default/ferromq.toml`; config switches
