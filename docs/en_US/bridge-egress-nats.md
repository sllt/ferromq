...
local *FerroMQ* forwards messages from the current cluster to the bridged remote *NATS* server.
...
```bash
plugins/ferromq-bridge-egress-nats.toml
```
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-egress-nats` entry to the
