...
*FerroMQ* allows for pre-setting permissions for clients during the authentication phase, which is used to control publish
...
```json
{
  ...
  "acl": [
    {
      // Allows the client to publish messages to the topic `foo/${clientid}`, for example, `foo/ferromq_c`.
      "permission": "allow",
      "action": "publish",
      "topic": "foo/${clientid}"
    },
    ...
    {
     // Allows the client to publish messages to the topic `foo/${username}` with `retain` equal to `false` and a QoS of 0 or 1, for example, `foo/ferromq_u`.
      "permission": "allow",
      "action": "publish",
      "topic": "foo/${username}",
      "retain": false,
...
If the `ferromq-acl.toml` configuration includes the rule `["allow", "all"]`, all ignored authentications will be allowed
