...
local FerroMQ forwards messages from the current cluster to the bridged remote Apache Pulsar server.
...
By default, this plugin is not enabled. To activate it, you must add the `ferromq-bridge-egress-pulsar` entry to the
