...
In ingress mode, the local *FerroMQ* consumes topics from the bridged remote *NATS* server and distributes the received messages within the current cluster.
...
```bash
plugins/ferromq-bridge-ingress-nats.toml
```
...
By default, this plugin is not enabled. To enable it, you must add the `ferromq-bridge-ingress-nats` entry in the `plugins.default_startups` section of the main configuration file `ferromq.toml`, for example:
