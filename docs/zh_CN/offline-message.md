...
即使 **未启用 `store-session` 插件**，FerroMQ 仍然支持离线消息存储。
...
若启用 **`ferromq-session-storage` 插件**，离线消息会随会话一起被持久化。
...
在主配置文件 **`ferromq.toml`** 中，可通过以下参数控制离线消息相关行为：
...
* **顺序与重复**：MQTT 协议允许消息重复投递，FerroMQ 会尽量保持顺序，客户端需支持幂等处理。
