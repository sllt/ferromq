# ferromq-test 按用例自动切换 Broker 配置

> 本文档记录 ferromq-test（`ferromq_harness`）单机测试框架的配置管理机制调整：
> **每个用例 / 每组用例可以使用自己的 broker 配置文件**，且测试配置全部
> **自包含**于 `ferromq-test/configs/`，不再依赖仓库根的 `ferromq.toml` 与
> `ferromq-plugins/*.toml`。
...
- `ferromq_harness` 启动 broker 时，仅当命令行给了 `--config` 才传 `-f <config>`，
  否则 `ferromqd` 直接读取**仓库根 `ferromq.toml`**；
...
- **配置自包含**：默认配置与各测试配置都放在 `ferromq-test/configs/<name>/`，
  主配置 + 自身 `plugins/` 子目录，不依赖仓库根。

## 3. 配置自包含（`configs/` 目录）

```
ferromq-test/configs/
  default/                  # 默认配置（未指定 --config 时使用）
    ferromq.toml              #   以仓库根 ferromq.toml 为蓝本
  ...
  tcp-keepalive/            # TCP keepalive 开启（issue #465 回归验证）
    ferromq.toml              #   tcp_keepalive = true
    plugins/
  pubrel-collision-cluster/ # 双节点集群（手动启动，1884/1885 MQTT）—— 不受本机制影响
```

### 3.1 `configs/default/ferromq.toml` 与仓库根配置的差异

| 配置项 | 仓库根值 | default 值 | 原因 |
|---|---|---|---|
| `plugins.dir` | `"ferromq-plugins/"` | `"ferromq-test/configs/default/plugins/"` | 自包含 |
| `listener.tcp.external.addr` | `"0.0.0.0:1883"` | `"127.0.0.1:1883"` | 匹配 harness 默认 `--addr` |
| `log.to` / `log.dir` | `"both"` / `/var/log/ferromq` | `"console"` / `"."` | 测试环境避免写系统目录 |
...
`ferromq-http-api` 插件在 Cargo.toml metadata 中 `default_startup = true`（与
`ferromq-acl`、`ferromq-counter` 一样会自动启动），且**强制要求存在配置文件**——
缺失时 `ferromqd` 注册插件直接失败（release 版 panic，debug 版卡死）。而
`ferromq-acl` / `ferromq-counter` 缺配置时仅 WARN 并使用默认值。

因此每个 `configs/<name>/plugins/` 目录内**完整拷贝** `ferromq-plugins/*.toml`
...
- 返回 `None` → 使用 harness 默认配置（`--config` 或 `configs/default/ferromq.toml`）；
- 返回路径 → 该用例声明需要特定配置，仅作为**构建期分组依据**，调度器不做用例级切换。

辅助函数 `tests::config_path(name)` 生成 `configs/<name>/ferromq.toml` 的绝对路径
...
`<workspace>/ferromq-test/configs/default/ferromq.toml`（**始终显式 `-f` 启动**，
不再存在"无配置"路径）。
...
- **workspace 解析**：`--workspace` 显式值 → cwd 探测（有 `ferromq.toml` 且
  `ferromq-test/configs/`）→ `CARGO_MANIFEST_DIR` 父目录（编译期仓库根）；
- **默认配置**：`--config` 或 `<workspace>/ferromq-test/configs/default/ferromq.toml`，
...
```
split suite 'functional_v5' -> 'functional_v5' (61 tests, config: .../configs/default/ferromq.toml)
...
...
switching broker config: ...default/ferromq.toml -> ...retain-disabled/ferromq.toml
...
1. 创建 `ferromq-test/configs/<name>/ferromq.toml`（以 `default/ferromq.toml` 为蓝本，
   修改 `plugins.default_startups` 与所需参数；监听端口保持 `127.0.0.1:1883`）；
2. 创建 `ferromq-test/configs/<name>/plugins/`，拷贝 `ferromq-plugins/*.toml`（26 个）；
...
- **工作目录**：`plugins.dir` 是相对进程 cwd 解析的，harness 与 ferromqd 都
  应从仓库根运行（`--workspace .`）。
...
| 文件 | 改动 |
|---|---|
| `ferromq-test/configs/default/ferromq.toml` + `plugins/*` | 新增：自包含默认配置 |
| `ferromq-test/configs/retain-disabled/`、`pubrel-collision/` | `plugins.dir` 指向自身 plugins/ + 补齐插件配置 |
...
- `cargo build -p ferromq-test` 零警告；`cargo test -p ferromq-test`（21 个 CLI 测试）全过；
...
1. **`ferromq-http-api` 插件强制要求配置文件**（`default_startup=true` 且缺失即
...
3. **验证环境坑（Windows）**：Git Bash 中 `taskkill //F` 双斜杠转义失效，
   杀不掉残留进程，需用 PowerShell `Stop-Process -Name ferromqd -Force`。
