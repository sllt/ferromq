...
本文档指导你搭建开发环境、编译 FerroMQ、运行测试并了解开发工作流。
...
生产二进制文件位于 `target/release/ferromqd`（Windows 上为 `ferromqd.exe`）。
...
cargo build -p ferromq     # 构建特定 crate
cargo test -p ferromq-codec # 运行 crate 的单元测试
```
...
# 默认使用自包含配置 ferromq-test/configs/default/ferromq.toml 启动 Broker
...
创建新插件：
1. 在 `ferromq-plugins/` 下创建新 crate
2. 添加到 `ferromq-plugins/Cargo.toml` 作为可选依赖
3. 在 `ferromq-plugins/src/lib.rs` 中添加 feature 标志和 re-export
