...
本文档描述了 FerroMQ 的测试策略、测试层次以及如何运行和扩展测试套件。
...
graph TD
    subgraph L1["第一层: 单元测试"]
        UT1["ferromq-codec 测试 v3/v5 编解码"]
        UT2["ferromq-net 测试 构建器 流"]
        UT3["ferromq-utils 测试 Bytesize NodeAddr 解析"]
        UT4["其他 crate 测试 cfg test 模块"]
    end

    subgraph L2["第二层: 集成测试"]
        IT1["ferromq_harness 5 套测试套件"]
...
`ferromq-test` crate 提供名为 `ferromq_harness` 的独立测试二进制文件。
...
> **Broker 配置**：默认使用自包含配置 `ferromq-test/configs/default/ferromq.toml`
> （不依赖仓库根的 `ferromq.toml` / `ferromq-plugins/*.toml`；保留 TCP/TLS/WS/WSS/QUIC
...
FerroMQ 通过了 [paho.mqtt.testing](https://github.com/eclipse/paho.mqtt.testing) 套件：
...
实现 `TestCase` trait 并在测试入口注册。详情见 [ferromq-test](../../../ferromq-test/README-CN.md)。
需要特殊 broker 配置的用例通过 `broker_config()` 声明，机制说明见
[ferromq-test 按用例自动切换 Broker 配置](./ferromq-test-config-switching.md)。
