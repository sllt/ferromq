...
本文档介绍如何为 FerroMQ 开发插件，涵盖插件生命周期、钩子系统、配置加载和最佳实践。

---

## 插件架构概述

FerroMQ 插件是独立的 Rust crate，实现 `Plugin` trait。它们可以通过钩子系统拦截 Broker 事件来扩展功能，无需修改核心代码。
...
use ferromq::plugin::{Plugin, Register};
use ferromq::hook::{self, Type, Handler};
...
1. 添加到 `ferromq-plugins/Cargo.toml` 作为可选依赖
2. 在 `ferromq-plugins/src/lib.rs` 添加 feature 标志和 re-export
3. 在 `ferromq-bin/Cargo.toml` 添加依赖和 `[package.metadata.plugins]` 配置
...
配置来源优先级：`{plugins.dir}/{name}.toml` → `ferromq_plugin_{name}_*` 环境变量 → 内联配置
