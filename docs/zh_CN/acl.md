...
内置 ACL 优先级最低，可以被 其它ACL 插件覆盖，如需禁用全部注释即可。规则文件更改后需重启 FerroMQ服务 以应用生效。
</font>
</div>
...
## ferromq-acl.toml 编写规则

`ferromq-acl.toml` 文件中的规则按书写顺序从上往下匹配。
...
在 `ferromq-acl.toml` 修改完成后，并不会自动加载至 FerroMQ 系统。需要手动执行：
...
内置的 `ferromq-acl.toml` 在主题的域（元组的第四位）仅支持以下占位符：
...
<div style="font-size:1.3em;">提示<br></div>
<font style="color:#435364;font-size:1.1em;">
ferromq-acl.toml 中应只包含一些简单而通用的规则，使其成为系统基础的 ACL 原则。如果需要支持复杂、大量的 ACL 内容，你应该在认证插件中去实现它。
</font>
</div>
