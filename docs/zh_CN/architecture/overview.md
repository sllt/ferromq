...
本文档描述了 FerroMQ MQTT Broker 的内部架构、组件组织、模块结构和关键设计决策。
...
| Crate | 路径 | 职责 |
|-------|------|------|
| `ferromq-utils` | `ferromq-utils/` | 共享类型（`Bytesize`、`NodeAddr`、`Counter`）、serde 辅助、时间戳/间隔解析 |
| `ferromq-macros` | `ferromq-macros/` | 过程宏：`#[derive(Metrics)]` 原子计数器、`#[derive(Plugin)]` PackageInfo trait |
| `ferromq-codec` | `ferromq-codec/` | MQTT 协议编解码 — v3.1、v3.1.1、v5.0 带版本协商 |
| `ferromq-storage` | `ferromq-storage/` | Sled、Redb、Redis 等键值存储的统一异步抽象 |
...
| Crate | 路径 | 依赖 | 职责 |
|-------|------|------|------|
| `ferromq-net` | `ferromq-net/` | `ferromq-codec`、`ferromq-utils` | 网络层：TCP/TLS/WS/QUIC 监听器、连接接受、协议分发 |
| `ferromq-conf` | `ferromq-conf/` | `ferromq-codec`、`ferromq-net`、`ferromq-utils`、`config` | 配置管理：TOML 解析、CLI 参数、监听器配置 |
| `ferromq-raft` | `ferromq-raft/` | `ferromq-raft-core`、Tonic、Postcard | Raft 封装、节点服务和集群 Mailbox |

### 第三层：核心 Broker

| Crate | 路径 | 依赖 | 职责 |
|-------|------|------|------|
| `ferromq` | `ferromq/` | 以上所有 + `ferromq-net`、`ferromq-codec`、`ferromq-utils`、`ferromq-macros`（可选）、`rust-box`、`dashmap`、`tokio` | 核心 MQTT Broker：会话管理、路由、钩子、插件、集群 |

### 第四层：二进制入口

| Crate | 路径 | 职责 |
|-------|------|------|
| `ferromqd` | `ferromq-bin/` | 生产二进制：CLI 解析 → 配置 → 插件注册 → 服务启动 |
| `ferromq_harness` | `ferromq-test/` | 测试框架：功能测试、压力测试、混沌测试 |

### 第五层：插件

| Crate | 路径 | 职责 |
|-------|------|------|
| `ferromq-plugins` | `ferromq-plugins/` | 元 crate，通过 feature 标志重新导出所有插件 |
| `ferromq-*` | `ferromq-plugins/ferromq-*/` | 26 个独立插件 crate |
...
钩子系统是 FerroMQ 的主要扩展机制，在消息处理管道的各个阶段提供了 **23 个拦截点**。
...
插件配置从 `{plugins.dir}/{name}.toml` 单独加载，支持 `ferromq_plugin_{name}_*` 环境变量覆盖。

---

## Feature 标志

核心 Broker（`ferromq`）使用 Cargo feature 标志条件编译可选功能：

| Feature | 启用内容 | 关键依赖 |
|---------|----------|----------|
| `default` | 最小构建（无额外功能） | — |
| `metrics` | 指标收集 | `ferromq-macros/metrics` |
| `stats` | 运行时统计 | — |
| `plugin` | 插件系统 | `ferromq-macros/plugin` |
| `grpc` | 节点间 gRPC | `rust-box/handy-grpc`、`msgstore` |
| `tls` | TLS 传输 | `ferromq-net/tls` |
| `ws` | WebSocket 传输 | `ferromq-net/ws` |
| `quic` | QUIC 传输 | `ferromq-net/quic` |
| `delayed` | 延迟发布 | — |
| `retain` | 保留消息 | — |
| `msgstore` | 消息存储 | — |
...
每个插件是独立的 crate，可选编译。元 crate（`ferromq-plugins`）通过 feature 标志重新导出所有插件，确保未使用的功能零开销。
