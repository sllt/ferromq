...
在入口模式下，本地的 *FerroMQ* 从桥接的远程 *Apache-Pulsar* 服务器消费主题，并在当前集群内分发接收到的消息。
...
默认情况下并没有启动此插件，如果要开启此插件，必须在主配置文件“ferromq.toml”中的“plugins.default_startups”配置中添加“ferromq-bridge-ingress-pulsar”项，如：
```bash
##--------------------------------------------------------------------
## Plugins
##--------------------------------------------------------------------
