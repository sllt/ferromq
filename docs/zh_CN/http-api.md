...
FerroMQ 提供了 HTTP API 以实现与外部系统的集成，例如查询客户端信息、发布消息等。

FerroMQ 的 HTTP API 服务默认监听 6060 端口，可通过 `etc/plugins/ferromq-http-api.toml` 配置文件修改监听端口。所有 API 调用均以 `api/v1` 开头。
...
FerroMQ 接口在调用成功时总是返回 200 OK，响应内容主要以 JSON 格式返回。
...
| .uptime        | String    | FerroMQ 运行时间，格式为 "D days, H hours, m minutes, s seconds" |
| .version       | String    | FerroMQ 版本                                               |
...
| ------ | --------- | -------- | ------- |  ---- |
| _limit | Integer   | False | 10000   | 一次最多返回的数据条数，未指定时由 `ferromq-http-api.toml` 插件的配置项 `max_row_limit` 决定 |
