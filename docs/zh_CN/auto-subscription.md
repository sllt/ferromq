...
自动订阅能够给 *FerroMQ* 设置多个规则，在设备成功连接后按照规则为其订阅指定主题，不需要额外发起订阅。

#### 配置自动订阅规则

*FerroMQ* 的自动订阅规则需要用户自行配置，用户可以自行添加多条自动订阅规则。
...
默认情况下并没有启动此插件，如果要开启此插件，必须在主配置文件“ferromq.toml”中的“plugins.default_startups”配置中添加“ferromq-auto-subscription”项，如：
```bash
##--------------------------------------------------------------------
## Plugins
##--------------------------------------------------------------------
