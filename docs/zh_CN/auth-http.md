...
> **提示**    
> ferromq-auth-http 插件同时包含 ACL 功能，可通过注释禁用。


## 认证原理

*FerroMQ* 在设备连接事件中使用当前客户端相关信息作为参数，向用户自定义的认证服务发起请求查询权限，通过返回的 HTTP 响应信息来处理认证请求。
...
从 *FerroMQ* v0.8.0 版本开始，您可以在响应体中设置一个可选的 acl 字段，用于指定客户端的权限。有关更多信息，请参阅[权限列表（ACL）](./perm-list.md)。

从 *FerroMQ* v0.8.0 版本开始，您可以在响应体中设置一个可选的 expire_at 字段，用于指定客户端的认证到期时间，并强制客户端断开连接以便重新认证。该值为 Unix 时间戳（秒）。
...
进行身份认证时，FerroMQ 将使用当前客户端信息填充并发起用户配置的认证查询请求，查询出该客户端在 HTTP 服务器端的认证数据。
...
你可以在认证请求中使用以下占位符，请求时 FerroMQ 将自动填充为客户端信息：
...
> **提示<br>**
> ferromq-auth-http 插件同时包含认证功能，可通过注释禁用。


要启用 HTTP ACL，需要在 `etc/plugins/ferromq-auth-http.toml` 中配置以下内容：

## ACL 授权原理

FerroMQ 在设备发布、订阅事件中使用当前客户端相关信息作为参数，向用户自定义的认证服务发起请求权限，通过返回的 HTTP 响应信息来处理 ACL 授权请求。
...
- 缓存授权结果：
  - 响应头返回 `X-Cache: -1` 表示结果已被缓存，值为缓存超时时间（单位：毫秒），`-1` 表示在连接存活期间一直有效。
  - **注意：当前订阅类型的 ACL 不支持缓存。**

进行发布、订阅认证时，FerroMQ 将使用当前客户端信息填充并发起用户配置的 ACL 授权查询请求，查询出该客户端在 HTTP 服务器端的授权数据。
