...
`ferromq-bridge-origin` 插件负责识别 MQTT 客户端是否来自 `ferromq-bridge-ingress-mqtt` 或 `ferromq-bridge-egress-mqtt` 插件建立的桥接连接。

当桥接插件（如 `ferromq-bridge-ingress-mqtt` 或 `ferromq-bridge-egress-mqtt`）连接到远程 MQTT 代理时，它们会生成带有可识别标记的客户端ID。例如：
...
`ferromq-bridge-origin` 插件监听客户端连接成功后的 `ClientConnected` 事件，检查 `client_id` 中是否包含可配置的标记，并将桥接来源信息写入 `session.extra_attrs`。其他插件可以读取这些元数据，用于防环或路由决策等场景。
...
读取时使用的键名必须与 `ferromq-bridge-origin` 中配置的 `attr_key` 一致（默认为 `"bridge_origin"`）：
...
这些标记与 `ferromq-bridge-ingress-mqtt` 和 `ferromq-bridge-egress-mqtt` 插件使用的自动生成客户端 ID 模式保持一致。
...
默认情况下并没有启动此插件，如果要开启此插件，必须在主配置文件 `ferromq.toml` 中的 `plugins.default_startups` 配置中添加 `ferromq-bridge-origin` 项，如：
```bash
##--------------------------------------------------------------------
## Plugins
##--------------------------------------------------------------------
