...
本文档提供 FerroMQ MQTT Broker 的详细测试结果，包括针对 paho.mqtt.testing 套件的互操作性测试和性能基准数据。

---

## 互操作性测试

FerroMQ 通过了官方 [paho.mqtt.testing](https://github.com/eclipse/paho.mqtt.testing) 互操作性测试套件。
...
| `test_unsubscribe` | ✅ 通过 | — |
| `test_subscribe_failure` | ✅ 通过 | 需在 `ferromq-acl.toml` 首行添加：`["deny", "all", "subscribe", ["test/nosubscribe"]]` |
| `test_zero_length_clientid` | ✅ 通过 | — |
...
| `test_will_delay` | ✅ 通过 |
| `test_server_keep_alive` | ✅ 通过（需将 `ferromq.toml` 中 `max_keepalive` 改为 60） |
...
`ferromq-test` crate 提供了自定义测试框架，在 paho 之外还包含以下套件：
...
> 默认 broker 配置为自包含的 `ferromq-test/configs/default/ferromq.toml`，
> 配置切换仅发生在 suite 边界。
