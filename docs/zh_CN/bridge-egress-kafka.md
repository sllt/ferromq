...
*Apache-Kafka*数据桥接是一种连接其他 *Apache-Kafka* 服务的方式。在出口模式下，本地的 FerroMQ 将当前集群中的消息转发给桥接的远程 *Apache-Kafka* 服务器。

### 并发连接：

*FerroMQ* 允许多个客户端同时连接到桥接的 *Apache-Kafka* 服务器。在创建桥接时您可以设置一个 *Apache-Kafka* 客户端并发连接数。合适的 *Apache-Kafka* 客户端并发连
...
```bash
plugins/ferromq-bridge-egress-kafka.toml
```
...
默认情况下并没有启动此插件，如果要开启此插件，必须在主配置文件“ferromq.toml”中的“plugins.default_startups”配置中添加“ferromq-bridge-egress-kafka”项，如：
```bash
##--------------------------------------------------------------------
## Plugins
##--------------------------------------------------------------------
