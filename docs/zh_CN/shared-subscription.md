...
此功能由 [ferromq-shared-subscription](https://github.com/rmqtt/rmqtt/tree/master/ferromq-plugins/ferromq-shared-subscription) 插件提供。
...
插件加载后，所有监听器自动支持共享订阅。在 `ferromq.toml` 的 `plugins.default_startups` 列表中添加 `ferromq-shared-subscription`：
