...
MQTT 数据桥接是一种连接多个 FerroMQ 集群或其他 MQTT 服务的方式。在出口模式下，本地的 FerroMQ 将当前集群中的消息转发给桥接的
远程 MQTT 服务器。
...
默认情况下并没有启动此插件，如果要开启此插件，必须在主配置文件“ferromq.toml”中的“plugins.default_startups”配置中添加“ferromq-bridge-egress-mqtt”项，如：
```bash
##--------------------------------------------------------------------
## Plugins
##--------------------------------------------------------------------
