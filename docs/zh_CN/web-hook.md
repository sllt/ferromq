...
WebHook 是由 [ferromq-web-hook](https://github.com/rmqtt/rmqtt/tree/master/ferromq-plugins/ferromq-web-hook) 插件提供的 **将 FerroMQ 中的钩子事件通知到某个 Web 服务** 的功能。

WebHook 的内部实现是基于 [钩子]，但它更靠近顶层一些。它通过在钩子上的挂载回调函数，获取到 FerroMQ 中的各种事件，并转发至 ferromq-web-hook 中配置的 Web 服务器。
...
WebHook 对于事件的处理是单向的，**它仅支持将 FerroMQ 中的事件推送给 Web 服务，并不关心 Web 服务的返回**。
...
Webhook 的配置文件位于 `etc/plugins/ferromq-web-hook.toml` 。
...
在 `etc/plugins/ferromq-web-hook.toml` 可配置触发规则，其配置的格式如下：
