...
use ferromq_net::{QuinnBiStream, Result};
...
async fn main() -> Result<()> {
    ...
    framed.send(MqttPacket::V3(ferromq_codec::v3::Packet::Connect(Box::new(connect)))).await?;
    framed.flush().await?;
    log::info!("Sent CONNECT");

    // Wait for CONNACK response
    if let Some(Ok((MqttPacket::V3(ferromq_codec::v3::Packet::ConnectAck(ack)), _))) = framed.next().await {
    ...
    framed.send(MqttPacket::V3(ferromq_codec::v3::Packet::Publish(Box::new(publish)))).await?;
    framed.flush().await?;
    log::info!("Sent PUBLISH");

    // Wait for PUBACK response
    if let Some(Ok((MqttPacket::V3(ferromq_codec::v3::Packet::PublishAck { packet_id }), _))) =
        framed.next().await
    {
