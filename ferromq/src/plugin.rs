...
macro_rules! register {
    ($name:path) => {
        ...
        pub async fn register_named(
            scx: &ferromq::context::ServerContext,
            name: &'static str,
            ...
            let scx1 = scx.clone();
            scx.plugins
                .register(name, default_startup, immutable, move || -> ferromq::plugin::DynPluginResult {
                    let scx1 = scx1.clone();
                    Box::pin(async move {
                        $name(scx1.clone(), name).await.map(|p| -> ferromq::plugin::DynPlugin { Box::new(p) })
                    })
                })
                .await?;
            Ok(())
...
/// Core trait for all FerroMQ plugins.
///
...
pub trait PackageInfo {
    /// Unique name of the plugin (e.g., `"ferromq-acl"`).
    fn name(&self) -> &str;
...
/// 1. Environment variables with prefix `ferromq_plugin_{name}`.
...
impl Manager {
    ...
    /// # Arguments
    ///
    /// * `name` — Plugin name (e.g., `"ferromq-acl"`).
    ...
    /// 1. Environment variables `ferromq_plugin_{name}_*`
    ...
    /// 3. Environment variables with prefix `ferromq_plugin_{name}`
    ///
