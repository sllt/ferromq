...
//!       async fn set(...) { /* delegates to ferromq-retainer plugin */ }
//!   }
//!   ```
...
//!
//! ## Usage Note
//! The base implementation intentionally delegates to `ferromq-retainer` plugin for:
//! - Distributed storage support
//! - Enhanced persistence mechanisms
//! - Cluster-wide message synchronization
//!
//! See `ferromq-retainer` documentation for production deployment recommendations
...
/// functional but logs a warning recommending the `ferromq-retainer`
...
/// Production deployments should use the `ferromq-retainer` plugin
...
impl RetainStorage for DefaultRetainStorage {
    ...
    async fn set(
        ...
        log::warn!("Please use the \"ferromq-retainer\" plugin as the main program no longer supports retain messages.");
        Ok(())
    }
